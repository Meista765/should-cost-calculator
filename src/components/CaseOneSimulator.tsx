...
import { findMaterialMeta } from '../lib/lookup';
import { formatKRW, formatKg, formatPercent } from '../lib/format';
import { DeltaBadge } from './DeltaBadge';

type Props = { asIs: UnifiedFormSlice; asIsBreakdown: CostBreakdown; db: Db };
...
function ThicknessTable({ asIs, asIsBreakdown, db }: Props) {
  if (!asIs.material || asIs.thkMm == null || asIs.volMm3 == null) {
    return <p className="empty-note">원소재 사양 정보를 입력하면 두께 변경 후보가 표시됩니다.</p>;
  }
  ...
  return (
    <div className="table-scroll">
      <table className="variant-table">
        ...
        <tbody>
          {variants.map((v) => {
            const isCurrent = v.thickness === asIs.thkMm;
            ...
            return (
              <tr key={v.thickness} className={isCurrent ? 'current' : ''}>
                ...
                <td className="num strong">{formatKRW(v.breakdown.materialCost)}</td>
                <td className="num">
...
function MaterialTable({ asIs, asIsBreakdown, db }: Props) {
  if (!asIs.material || asIs.thkMm == null) {
  ...
  return (
    <div className="table-scroll">
      <table className="variant-table">
        ...
        <tbody>
          {variants.map((v) => {
            const isCurrent = v.grade === asIs.material;
            const meta = findMaterialMeta(v.grade, db);
            return (
              <tr key={v.grade} className={isCurrent ? 'current' : ''}>
                <td>{v.displayName}</td>
                <td className="num">{meta ? meta.density.toFixed(2) : '—'}</td>
                <td className="num strong">{formatKRW(v.breakdown.materialCost)}</td>
                <td className="num">
                  {isCurrent ? <span className="muted">현재</span> : <DeltaBadge value={v.deltaTotal} />}
                </td>
